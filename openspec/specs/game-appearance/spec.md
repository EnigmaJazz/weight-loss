...
- THEN the Summary card MUST contain the hero ring, the Streaks tiles MUST keep `.flame` and `dataset.streakActive` with a larger flame + count, and the Checkpoints card MUST contain the five-card track, with no new tab introduced## Extended by r2-world-xp-island (2026-08-13)
